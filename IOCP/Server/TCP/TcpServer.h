...
public:
	...
	virtual BOOL			Stop();
	virtual BOOL			Send(ULONG_PTR dwConnID, const BYTE* pBuffer, int iLength, int iOffset = 0);
	virtual BOOL			SendSmallFile(ULONG_PTR dwConnID, LPCTSTR lpszFileName, const LPWSABUF pHead = nullptr, const LPWSABUF pTail = nullptr);
	virtual BOOL			SendPackets(ULONG_PTR dwConnID, const WSABUF pBuffers[], int iCount) { return DoSendPackets(dwConnID, pBuffers, iCount); }
	virtual BOOL			PauseReceive(ULONG_PTR dwConnID, BOOL bPause = TRUE);
	virtual BOOL			Wait(DWORD dwMilliseconds = INFINITE) { return m_evWait.Wait(dwMilliseconds); }
	virtual BOOL			HasStarted() { return m_enState == SS_STARTED || m_enState == SS_STARTING; }	
	virtual BOOL			Disconnect(ULONG_PTR dwConnID, BOOL bForce = TRUE);
	...
	virtual BOOL			GetLocalAddress(ULONG_PTR dwConnID, TCHAR lpszAddress[], int& iAddressLen, USHORT& usPort);
	virtual BOOL			GetRemoteAddress(ULONG_PTR dwConnID, TCHAR lpszAddress[], int& iAddressLen, USHORT& usPort);
	virtual BOOL			IsConnected(ULONG_PTR dwConnID);
	virtual BOOL			IsPauseReceive(ULONG_PTR dwConnID, BOOL& bPaused);
	virtual BOOL			GetPendingDataLength(ULONG_PTR dwConnID, int& iPending);	
	virtual BOOL			GetAllConnectionIDs(ULONG_PTR pIDs[], DWORD& dwCount);
	virtual BOOL			GetConnectPeriod(ULONG_PTR dwConnID, DWORD& dwPeriod);
	virtual BOOL			GetSilencePeriod(ULONG_PTR dwConnID, DWORD& dwPeriod);
...
public:
	...
	virtual BOOL SetConnectionExtra(ULONG_PTR dwConnID, PVOID pExtra);
	virtual BOOL GetConnectionExtra(ULONG_PTR dwConnID, PVOID* ppExtra);
...
protected:
	...
	virtual void OnWorkerThreadStart(DWORD dwThreadID) {}
	virtual void OnWorkerThreadEnd(DWORD dwThreadID) {}

	BOOL DoSendPackets(ULONG_PTR dwConnID, const WSABUF pBuffers[], int iCount);
	BOOL DoSendPackets(TSocketObj* pSocketObj, const WSABUF pBuffers[], int iCount);
	TSocketObj* FindSocketObj(ULONG_PTR dwConnID);
...
protected:
	...
	BOOL SetConnectionReserved(ULONG_PTR dwConnID, PVOID pReserved);
	BOOL GetConnectionReserved(ULONG_PTR dwConnID, PVOID* ppReserved);
	...
	BOOL SetConnectionReserved2(ULONG_PTR dwConnID, PVOID pReserved2);
	BOOL GetConnectionReserved2(ULONG_PTR dwConnID, PVOID* ppReserved2);
...
private:
	...
	TSocketObj* GetFreeSocketObj(ULONG_PTR dwConnID, SOCKET soClient);
	...
	void		AddClientSocketObj(ULONG_PTR dwConnID, TSocketObj* pSocketObj, const _SOCKADDR& remoteAddr);
...
private:
	...
	void ForceDisconnect(ULONG_PTR dwConnID);
	void HandleIo(ULONG_PTR dwConnID, TSocketObj* pSocketObj, TBufferObj* pBufferObj, DWORD dwBytes, DWORD dwErrorCode);
	void HandleError(ULONG_PTR dwConnID, TSocketObj* pSocketObj, TBufferObj* pBufferObj, DWORD dwErrorCode);
	void HandleAccept(SOCKET soListen, TBufferObj* pBufferObj);
	void HandleSend(ULONG_PTR dwConnID, TSocketObj* pSocketObj, TBufferObj* pBufferObj);
	void HandleReceive(ULONG_PTR dwConnID, TSocketObj* pSocketObj, TBufferObj* pBufferObj);
	...
	BOOL DoAccept();
	int DoUnpause(ULONG_PTR dwConnID);
	int DoReceive(TSocketObj* pSocketObj, TBufferObj* pBufferObj);
	int DoSend(ULONG_PTR dwConnID);
